...
u80 = fmincon(@(x)(MaxCDF(x,nSub-1,nVox)-0.80).^2,mean(MnMx),[],[],[],[],MnMx(1),MnMx(2),[],optopt);
u90 = fmincon(@(x)(MaxCDF(x,nSub-1,nVox)-0.90).^2,mean(MnMx),[],[],[],[],MnMx(1),MnMx(2),[],optopt);
u95 = fmincon(@(x)(MaxCDF(x,nSub-1,nVox)-0.95).^2,mean(MnMx),[],[],[],[],MnMx(1),MnMx(2),[],optopt);
...
%set(h,'linestyle','-');
uistack(h,'top')
h2=plot(xx,MaxCDF(xx,nSub-1,nVox),'color',[0.8 0 0 ],'linewidth',2);
