...
for t=1:nRlz
    fprintf('.');
      ...
      for k=1:nBoot 
          ...
          observed_lshift_weighted_boundary_values = abs(observed_lshift_w1.*resid_field(observed_lshift) + observed_lshift_w2.*resid_field(observed_lshift(:,[dim(2) 1:dim(2)-1])));
          observed_rshift_weighted_boundary_values = abs(observed_rshift_w1.*resid_field(observed_rshift) + observed_rshift_w2.*resid_field(observed_rshift(:,[2:dim(2) 1])));
          observed_ushift_weighted_boundary_values = abs(observed_ushift_w1.*resid_field(observed_ushift) + observed_ushift_w2.*resid_field(observed_ushift([dim(1) 1:dim(1)-1],:)));
          observed_dshift_weighted_boundary_values = abs(observed_dshift_w1.*resid_field(observed_dshift) + observed_dshift_w2.*resid_field(observed_dshift([2:dim(1) 1],:)));
    ...
    if sum(upper_subset_mid_raw_80_weighted(:))+sum(mid_subset_lower_raw_80_weighted(:))==0
      ...
      fprintf('raw nominal 80 weighted boundary success! \n');
    else 
      subset_success_vector_raw_80_weighted(t) = 0; 
      fprintf('raw nominal 80 weighted boundary failure! \n');
    end 
    ...
    if sum(upper_subset_mid_raw_80_observed_weighted(:))+sum(mid_subset_lower_raw_80_observed_weighted(:))==0
      ...
      fprintf('raw nominal 80 observed_weighted boundary success! \n');
    else 
      subset_success_vector_raw_80_observed_weighted(t) = 0; 
      fprintf('raw nominal 80 observed_weighted boundary failure! \n');
    end 
