...
for t=1:nRlz
    fprintf('.');
      ...
      snr_resid = spdiags(1./sqrt(1 + 0.5*reshape(observed_d, [prod(dim) 1]).^2), 0, prod(dim), prod(dim))*snr_resid; 
      ...
      for k=1:nBoot 
          ...
          snr_resid_bootstrap  = snr_resid*spdiags(signflips, 0, nSubj, nSubj);
          ...
          supG_cohen(k)        = max(abs(snr_resid_field(true_boundary)));
    ...
    supGa_raw                     = prctile(supG_raw, 95);
    supGa_cohen                   = prctile(supG_cohen, 95);
